...
(function () {
    ...
    async function loadFileList() {
        ...
        try {
            ...
            files.forEach(function (f) {
                ...
                const icon = type === 'puml' ? '📊' : type === 'adoc' ? '📄' : '📃';
                const btn = document.createElement('button');
                btn.type = 'button';
                btn.className = 'file-item';
                btn.dataset.fileName = name;
                btn.dataset.fileType = type;
                btn.innerHTML = '<span class="file-icon">' + escapeHtml(icon) + '</span><span class="file-name">' + escapeHtml(name) + '</span>';
    ...
    /** Убирает обёртку ```asciidoc ... ``` из текста, чтобы рендерить чистый AsciiDoc. */
    function stripAsciiDocFences(text) {
        var t = (text || '').trim();
        var open = /^```\s*asciidoc\s*\n/i;
        if (open.test(t)) {
            t = t.replace(open, '');
            var close = /\n```\s*$/;
            if (close.test(t)) t = t.replace(close, '');
        }
        return t.trim();
    }

    /** Извлекает содержимое body из полного HTML документа (ответ Asciidoctor standalone). */
    function extractBodyHtml(html) {
        if (!html || typeof html !== 'string') return html || '';
        var s = html.trim();
        var bodyMatch = /<body[^>]*>([\s\S]*?)<\/body>/i.exec(s);
    ...
    async function showAdocAsDocument() {
        ...
        try {
            ...
            renderedEl.innerHTML = extractBodyHtml(rawHtml);
        } catch (e) {
